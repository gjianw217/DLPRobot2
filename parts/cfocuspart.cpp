...
#include "cfocuspart.h"

CFocusPart::CFocusPart(PCDataMapping datamapping):IParts(datamapping)
{

}

CFocusPart::~CFocusPart()
{

}

void CFocusPart::Init()
{
    ...
    {
        dlp_log(DLP_LOG_DEBUG,"CFocusPart Create Coder");
        IParts::CreateCoder(RS485_CODER_BOCHEN,DLP_FOCUS);
    ...
    {
        dlp_log(DLP_LOG_DEBUG,"CFocusPart Create Motor");
        IParts::CreateMotor(STEP_MOTOR_BY_PWM,DLP_FOCUS);
...
void CFocusPart::parseCoderAttr(DLPEncoderAttr &attr)
{
    uint16_t data[10]={0};
    //m_pdatamapping->GetHoldRegisters(0x500,8,data);
    m_pdatamapping->GetHoldRegisters(FOCUSCODER,8,data);//FOCUSCODER==0x500
    attr.dlp_id=data[0];
...
void CFocusPart::parseMotorAttr(DLPMotorAttr &attr)
{
    ...
    m_pdatamapping->GetInputRegisters(FOCUSMOTOR+5,4,data);
